...
static void generate_prototypes(FILE *file, const struct struct_def *s, const char *decl/*NULL?*/, int w)
{
	...
	if (s->need_destroy) {
		if (w & 4)
			FR2("\n#define _%s_ac_destroy       BRIDGE_DEBUG_SUFFIX(_%s_ac_destroy)", s->s_name, s->s_name);
		if (w & 4)
			FR2("\n#define _%s_ac_delete        BRIDGE_DEBUG_SUFFIX(_%s_ac_delete)", s->s_name, s->s_name);
		if (w & 4) {
	...
	if (w & 4)
		FR2("\n#define _%s_pack             BRIDGE_DEBUG_SUFFIX(_%s_pack)", s->s_name, s->s_name);
	if (w & 4)
		FR2("\n#define _%s_unpack_to        BRIDGE_DEBUG_SUFFIX(_%s_unpack_to)", s->s_name, s->s_name);
	if (w & 1) {
		FR4("\n"
...
static void print_source_includes(FILE *file, const char *cname, const char *hname)
{
	FR3("/* %s */"
		"\n"
		"\n/* generated by %s */"
...
FILE *generate_c(FILE *file, const char *cname, const char *hname, const char *decl/*NULL?*/)
{
	{
		size_t ch = strlen(hname), cs = strlen(cname);
		for (; ch && hname[ch - 1] != '\\' && hname[ch - 1] != '/'; ch--);
		for (; cs && cname[cs - 1] != '\\' && cname[cs - 1] != '/'; cs--);
		print_source_includes(file, cname + cs, hname + ch);
	}
