...
LIB         := $(if $(VARIANTS),bridge_test_lib $(firstword $(filter-out R,$(VARIANTS))))
endif

ifndef NO_SHARED
VARIANTS    := $(call FILTER_VARIANTS_LIST,DLL,$(BITBRIDGE_DLL_VARIANTS))
DLL         := $(if $(VARIANTS),bridge_test_dll $(firstword $(filter-out R,$(VARIANTS))))
DLL_DLLS    := $(BITBRIDGE_LIB_NAME)$(call DLL_VAR_SUFFIX,DLL,$(firstword $(filter-out R,$(VARIANTS))),$(VARIANTS))
else ifndef NO_STATIC
VARIANTS    := $(filter D,$(call FILTER_VARIANTS_LIST,LIB,$(BITBRIDGE_LIB_VARIANTS)))
DLL         := $(if $(VARIANTS),bridge_test_dll)
DLL_LIBS    := $(BITBRIDGE_LIB_NAME)
endif
