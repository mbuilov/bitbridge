...
TRG_VARS := PCH WITH_PCH SRC SDEPS DEFINES INCLUDE CFLAGS CXXFLAGS ASMFLAGS LDFLAGS SYSLIBS SYSLIBPATH SYSINCLUDE DLLS LIBS USE
...
$(eval $(OSVARS))
$(eval $(foreach t,$(BLD_TARGETS),$(if $($1),$(if $(MDEBUG),$(if $($t_USE),$$(info \
  $t using: $($t_USE))))$(newline)TAR := $t$(newline)include $(addprefix $(PROJECT_USE_DIR)/,$($t_USE)))))
